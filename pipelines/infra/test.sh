...
terraform init -backend-config="storage_account_name=${PREFIX}${ENV}tfsa" -backend-config="container_name=terraform-state" -backend-config="access_key=$SA_ACCOUNTKEY" -backend-config="key=terraform.tfstate"  -backend-config="resource_group_name=${PREFIX}-${ENV}-tf-rg"
