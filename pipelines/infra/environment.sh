...
## but since the permissions to the terraform SP are only given at rg level instead of subscriptoin level, we need to create them in order to delegate access :):)
